...
<!--                            <button value="button" class="hover:text-gray-300 bg-yellow-800 text-white hover:bg-black w-56 p-4 text-2xl font-bold" id="moka-8pwrq" style="font-family: Barlow;">시작</button>-->
                            ...
                            hover:text-white hover:bg-red-600 active:bg-red-700 focus:ring-red-300 inline-flex m-2" type="submit">나가기</button>
                            </div>
                ...
                <div class="flex flex-col items-center justify-center">
                    <div class="">
                        ...
                        inline-flex grid grid-cols-4 gap-4" v-for="(player, index) in players" :key="index">
                            <div class="grid grid-cols-1 gap-1">
                                <div class="bubble text-white w-40 h-9" v-if="chat">
                                    <h3 class="m-1 font-black">{{this.chat_log.n1}}</h3>
                                </div>
                                <div class="bubble2 h-9" v-if="chat===false">
                                </div>
                                <div class="card text-center shadow-2xl bg-gray-500 inline-flex m-1">
                                    <div>
                                        ...
                                        <div class="card-body z-0">
                                            <h2 class="card-title">{{player.user.name}}</h2>
                                            <p class="text-center">{{player.user.email}}</p>
                                            <div class="form-control mt-2">
                                                <input type="text" placeholder="엔터눌러" class="input input-info input-bordered"
                                                       v-model="this.chat_input_log.n1" @keypress.enter="sendMessage">
                                            </div>
                                        </div>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        sendMessage() {
            ...
            axios.post('/api/game/messages/' + this.room_info.id, {'message' : this.chat_log.n1})
                .then(response => {
